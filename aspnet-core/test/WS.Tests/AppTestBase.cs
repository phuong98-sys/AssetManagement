...
namespace WS.Tests
{
    ...
    /// Provides methods to easily work with <see cref="WSDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<WSTestModule>
    {
        ...
        {
            void NormalizeDbContext(WSDbContext context)
            {
        ...
        protected void UsingDbContext(Action<WSDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<WSDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<WSDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<WSDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<WSDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<WSDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<WSDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<WSDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<WSDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<WSDbContext, Task<T>> func)
        {
            T result;
